...
def analyseAndUpdate(my_database, job_details_list,
    ...
    my_database.update_stats(
        increment, my_database.cloud_data_ref)

    # analyse database
    increment = db_count(job_details_list)
    my_database.update_stats(increment, my_database.db_data_ref)

    # analyse language
    increment = lang_count(job_details_list)
    my_database.update_stats(increment,
                             my_database.lang_data_ref)

    # analyse libraries
    increment = lib_count(job_details_list)
    my_database.update_stats(increment, my_database.lib_data_ref)

    # analyse location
    increment = location_count(location_list)
    my_database.update_stats(increment, my_database.loc_data_ref)

    # analyse os
    increment = os_count(job_details_list)
    my_database.update_stats(increment, my_database.os_data_ref)
    ...
    my_database.update_stats(increment, my_database.web_data_ref)
