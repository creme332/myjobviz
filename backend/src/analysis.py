...
def update_analytics(main_db: Database, job_details_list: list[str],
                     location_list: list[str], salary_list: list[str]) -> None:
    # analyse cp
    increment = count_occurences(job_details_list, CLOUD_PLATFORMS, cp_check)
    main_db.update_stats(
        increment, main_db.cloud_data_ref)

    # analyse database
    increment = count_occurences(job_details_list, DATABASES, db_check)
    main_db.update_stats(increment, main_db.db_data_ref)

    # analyse language
    increment = count_occurences(job_details_list, LANGUAGES, language_check)
    main_db.update_stats(increment,
                         main_db.lang_data_ref)

    # analyse libraries
    increment = count_occurences(job_details_list, LIBRARIES, libraries_check)
    ...
    # analyse os
    increment = count_occurences(job_details_list, OPERATING_SYSTEMS, os_check)
    ...
    # analyse tools
    increment = count_occurences(job_details_list, TOOLS, tools_check)
    main_db.update_stats(
        increment, main_db.tools_data_ref)

    # analyse web frameworks
    increment = count_occurences(
        job_details_list, WEB_FRAMEWORKS, web_framework_check)
